...
public class ConceptsApiController implements ConceptsApi {
    ...
    public ResponseEntity<ServerConceptsQueryStatus1> getConceptsQueryStatus(@ApiParam(value = "an active query identifier as returned by a POST of concept query parameters.",required=true ) @PathVariable("queryId") String queryId,
        ...
        // do some magic!
        return new ResponseEntity<ServerConceptsQueryStatus1>(HttpStatus.OK);
    }

    public ResponseEntity<ServerConceptsQueryStatus> postConceptsQuery( @NotNull @ApiParam(value = "a (urlencoded) space delimited set of keywords or substrings against which to match concept names and synonyms, e.g. diabetes.", required = true) @RequestParam(value = "keywords", required = true) String keywords,
