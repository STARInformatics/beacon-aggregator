...
import bio.knowledge.server.model.ServerConceptsQueryStatus1;
...
public interface ConceptsApi {
    ...
    @ApiOperation(value = "", notes = "Retrieves the status of a given query about the concepts in the system ", response = ServerConceptsQueryStatus1.class, tags={ "concepts", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Current query status returned ", response = ServerConceptsQueryStatus1.class) })
    @RequestMapping(value = "/concepts/status/{queryId}",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<ServerConceptsQueryStatus1> getConceptsQueryStatus(@ApiParam(value = "an active query identifier as returned by a POST of concept query parameters.",required=true ) @PathVariable("queryId") String queryId,
    ...
    @ApiOperation(value = "", notes = "Posts the query parameters to retrieves a (paged) list of  concepts from the system ", response = ServerConceptsQueryStatus.class, tags={ "concepts", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful concept query initialization, with initial query status returned ", response = ServerConceptsQueryStatus.class) })
    @RequestMapping(value = "/concepts",
        produces = { "application/json" }, 
        method = RequestMethod.POST)
    ResponseEntity<ServerConceptsQueryStatus> postConceptsQuery( @NotNull @ApiParam(value = "a (urlencoded) space delimited set of keywords or substrings against which to match concept names and synonyms, e.g. diabetes.", required = true) @RequestParam(value = "keywords", required = true) String keywords,
