...
public interface ConceptsApi {
    ...
    @ApiOperation(value = "", notes = "Retrieves details for a specified concepts in the system, as specified by a (url-encoded) CURIE identifier of a concept known the given knowledge source ", response = ServerConceptDetail.class, responseContainer = "List", tags={ "concepts", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful response with concept details returned ", response = ServerConceptDetail.class) })
    @RequestMapping(value = "/concepts/{conceptId}",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<List<ServerConceptDetail>> getConceptDetails(@ApiParam(value = "(url-encoded) CURIE identifier of concept of interest, e.g. wd:Q126691",required=true ) @PathVariable("conceptId") String conceptId,
